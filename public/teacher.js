...
const playerCountSpan = document.getElementById('player-count'); // 關鍵：顯示人數的元素
...
const selFateCount = document.getElementById('sel-fate-count'); // 關鍵：如果 HTML 沒更新，這裡會抓不到
...
if (!playerCountSpan) console.error("❌ 找不到 id='player-count'，請檢查 teacher.html");
if (!selFateCount) console.warn("⚠️ 找不到 id='sel-fate-count'，請確認 teacher.html 是否已更新");
...
// --- Socket 事件監聽 ---
socket.on('connect', () => { 
    if(connectionStatus) { connectionStatus.innerText = "🟢 伺服器已連線"; connectionStatus.style.color = "#2ecc71"; }
    socket.emit('admin_login'); 
});
socket.on('disconnect', () => { 
    if(connectionStatus) { connectionStatus.innerText = "🔴 與伺服器斷線"; connectionStatus.style.color = "#e74c3c"; }
});

// 🔥 更新玩家列表與人數 (加入 Debug)
socket.on('update_player_list', (players) => { 
    console.log("Receive update_player_list:", players);
    updateView(players); 
});

// 🔥 更新遊戲狀態 (加入安全檢查)
socket.on('update_game_state', (gameState) => {
    console.log("Receive update_game_state:", gameState);
...
if(startBtn) startBtn.addEventListener('click', () => {
    ...
    // 安全讀取數值
    const trapEnabled = chkTrap ? chkTrap.checked : false;
    const fateCountVal = selFateCount ? selFateCount.value : 0;

    const options = {
        enableTraps: trapEnabled,
        fateCount: fateCountVal
    };
...
if(resetBtn) resetBtn.addEventListener('click', () => {
    showModal("危險操作", "確定要踢除所有玩家並回到首頁嗎？\n(若只是要重玩，請按「下一局」)", true, () => {
        ...
        if(liveMsg) liveMsg.innerText = "等待學生加入...";
        SynthEngine.stopBGM();
    });
});
